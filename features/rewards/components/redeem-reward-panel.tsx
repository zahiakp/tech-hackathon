...
export function RedeemRewardPanel({ reward, balance }: { reward: Reward; balance: number }) {
  ...
  return (
    <div className="grid gap-6">
      <PreviewAlert description="Redemption is a UI preview. Points are not deducted and no claim is created." />
      ...
      <Card>
        ...
        <CardContent className="grid gap-5">
          ...
          <ConfirmDialog trigger={<Button className="w-full"><Sparkles />Confirm redemption</Button>} title={`Redeem ${reward.title}?`} description={`This preview confirms a ${reward.cost}-point redemption. No points will actually be deducted.`} confirmLabel="Preview redemption" onConfirm={() => setComplete(true)} />
        </CardContent>
      </Card>
    </div>
  );
