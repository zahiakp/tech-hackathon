...
const rewardLinks = [
  { href: "/rewards/history", title: "Points history", description: "Review earned and redeemed points.", icon: History },
...
export default function RewardsPage() {
  ...
  return (
    <div className="grid gap-6">
      <PageHeader action={<Link className={buttonVariants()} href="/rewards/catalog"><Gift />Browse rewards</Link>} description="Track campus contributions, spend points, and celebrate achievements." eyebrow="Student rewards" title="Points & rewards" />
      <PreviewAlert description="Point balances, rewards, and claims are preview data and are not connected to a rewards service." />
      <div className="grid gap-4 sm:grid-cols-3"><DataCard className="sm:col-span-2" title="Available balance" value={pointBalancePreview.toLocaleString()} description="Points ready to redeem" icon={Sparkles} /><DataCard title="Earned this month" value={`+${earnedThisMonth}`} description={`${rewardsPreview.length} rewards available`} icon={Award} /></div>
