...
export default function LibraryFinesPage() {
  ...
  return <div className="grid gap-6"><PageHeader description="Review outstanding and resolved library charges." eyebrow="My library" title="Fine details" /><div className="grid gap-4 sm:grid-cols-2"><DataCard title="Outstanding total" value={`₹${total}`} description={`${due.length} fine requiring attention`} icon={CircleDollarSign} /><DataCard title="Overdue items" value={1} description="Return overdue books to prevent additional charges" icon={AlertTriangle} /></div><FineList books={libraryBooks} fines={libraryFines} /></div>;
}
